...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=None, tty="/dev/ttyACM0", baud=9600, maxvaluescount=3, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        ...
        self.valuesUltrasoundRawData = collections.deque([[0]*10]*maxvaluescount, maxlen=maxvaluescount)
...
def test_main():
    ...
    #pi = pigpio.pi()
    #hedge = PigpioMindHedge(pi, tty = "/dev/ttyACM0", adr=None, debug=True)
    hedge = MarvelmindHedge(tty = "/dev/ttyACM0", adr=None, debug=True) # create MarvelmindHedge thread

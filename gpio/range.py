# -*- coding: utf-8 -*-
"""
Agent Jones に搭載可能な距離計測センサをあらわすパーツクラス。
"""
import pigpio
import time
...
class RangeSensor:
    """
    Agent Jonesに接続された距離計測センサをあらわすパーツクラス。
    """
    def __init__(self, pi, range_gpios):
        ...
        if range_gpios is not None:
            from .hc_sr04 import SonarRangeDriver
            self.range = SonarRangeDriver(self.pi, range_gpios[0], range_gpios[1])
        else:
            self.range = None
        self.distance = None

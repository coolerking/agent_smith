...
class HC_SR04:
    ...
    def __init__(self, pgio, trigger, echo):
        """
...
def get_range_part(cfg, pgio=None):

    if cfg.RANGE_TYPE == "HC_SR04":
        cont_class = HC_SR04
        ctr = cont_class(pgio=pgio, trigger=cfg.RANGE_TRIG_GPIO, echo=cfg.RANGE_ECHO_GPIO)
        return ctr
    else:
        raise ValueError('no match range driver for RANGE_TYPE = {}'.format(str(cfg.RANGE_TYPE)))